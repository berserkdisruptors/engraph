...
describe("computeDependencyDirection", () => {
  it("identifies leaf modules", () => {
    const modules: Module[] = [
      createModule({
        ...
        imports: { internal: [], external: [] },
        imported_by: [
          { module_id: "commands", import_count: 1, symbols: [] },
        ],
      }),
    ];

    const result = computeDependencyDirection(modules);
    expect(result.leafModules).toContain("validation");
  });

  it("does not classify modules with outbound deps as leaves", () => {
    const modules: Module[] = [
      createModule({
        id: "commands",
        imports: {
          internal: [
            { module_id: "utils", import_count: 1, symbols: [] },
          ],
          external: [],
        },
        imported_by: [
          { module_id: "root", import_count: 1, symbols: [] },
        ],
      }),
    ];

    const result = computeDependencyDirection(modules);
    expect(result.leafModules).not.toContain("commands");
  });

  it("identifies hub modules with inbound >= 3", () => {
    const modules: Module[] = [
      createModule({
        id: "utils",
        imported_by: [
          { module_id: "a", import_count: 1, symbols: [] },
          { module_id: "b", import_count: 1, symbols: [] },
          { module_id: "c", import_count: 1, symbols: [] },
          { module_id: "d", import_count: 1, symbols: [] },
        ],
      }),
    ];

    const result = computeDependencyDirection(modules);
    expect(result.hubModules).toHaveLength(1);
    expect(result.hubModules[0].moduleId).toBe("utils");
    expect(result.hubModules[0].inboundCount).toBe(4);
  });

  it("sorts hub modules by inbound count descending", () => {
    const modules: Module[] = [
      createModule({
        id: "a",
        imported_by: [
          { module_id: "x", import_count: 1, symbols: [] },
          { module_id: "y", import_count: 1, symbols: [] },
          { module_id: "z", import_count: 1, symbols: [] },
        ],
      }),
      createModule({
        id: "b",
        imported_by: [
          { module_id: "w", import_count: 1, symbols: [] },
          { module_id: "x", import_count: 1, symbols: [] },
          { module_id: "y", import_count: 1, symbols: [] },
          { module_id: "z", import_count: 1, symbols: [] },
          { module_id: "v", import_count: 1, symbols: [] },
        ],
      }),
    ];

    const result = computeDependencyDirection(modules);
    expect(result.hubModules[0].moduleId).toBe("b");
    expect(result.hubModules[1].moduleId).toBe("a");
  });
  ...
  it("detects circular dependencies", () => {
    const modules: Module[] = [
      createModule({
        id: "a",
        imports: {
          internal: [{ module_id: "b", import_count: 1, symbols: [] }],
          external: [],
        },
        imported_by: [
          { module_id: "b", import_count: 1, symbols: [] },
      ...
      createModule({
        id: "b",
        imports: {
          internal: [{ module_id: "a", import_count: 1, symbols: [] }],
          external: [],
        },
        imported_by: [
          { module_id: "a", import_count: 1, symbols: [] },
